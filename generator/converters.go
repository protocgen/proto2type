...
func generateFromProto(g *protogen.GeneratedFile, dm *DomainMessage, structSuffix string, opts *Options) {
	...
	g.P("//")
	g.P("// When msg is non-nil, the receiver is fully reset before population: all")
	g.P("// reference fields (slices, maps, pointers, nested messages) are cleared to")
	g.P("// their zero values, and scalar fields are overwritten unconditionally. This")
	g.P("// makes it safe to reuse a receiver across multiple FromProto calls without")
	g.P("// retaining stale data. When msg is nil, the receiver is left unchanged.")
