...
func generateFromProto(g *protogen.GeneratedFile, dm *DomainMessage, structSuffix string, opts *Options) {
	...
	g.P("//")
	g.P("// Note: The receiver is not fully zeroed before population. Repeated and map")
	g.P("// fields are overwritten only when the source message has non-empty values.")
	g.P("// To avoid retaining stale data from a previous call, use a fresh (zero-value)")
	g.P("// receiver rather than reusing one across multiple FromProto calls.")
