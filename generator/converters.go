...
func generateFromProto(g *protogen.GeneratedFile, dm *DomainMessage, structSuffix string, opts *Options) {
	...
	g.P("//")
	g.P("// The receiver is fully reset before population: all reference fields")
	g.P("// (slices, maps, pointers, nested messages) are cleared to their zero values,")
	g.P("// and scalar fields are overwritten unconditionally. This makes it safe to")
	g.P("// reuse a receiver across multiple FromProto calls without retaining stale data.")
