...
func generateTypeScriptDomain(gen *protogen.Plugin, file *protogen.File, opts *Options) error {
	...
	for _, f := range gen.Files {
		if f.Generate {
			generatedPaths[f.Desc.Path()] = true
		}
	}
	...
	writeTSFile(g, ir, opts, generatedPaths)
	return nil
}

func writeTSFile(g *protogen.GeneratedFile, ir *DomainFile, opts *Options, generatedPaths map[string]bool) {
	...
	imports := collectTSImports(ir, opts, generatedPaths)
