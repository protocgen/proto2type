...
func TestTsZodConstraints_BytesLength(t *testing.T) {
	...
	opts := &Options{}
	got := tsZodConstraints(f, opts)
	expected := `.refine(v => { const p = v.endsWith("==") ? 2 : v.endsWith("=") ? 1 : 0; return (v.length * 3 / 4) - p >= 5; }, { message: "bytes must be at least 5 bytes" }).refine(v => { const p = v.endsWith("==") ? 2 : v.endsWith("=") ? 1 : 0; return (v.length * 3 / 4) - p <= 10; }, { message: "bytes must be at most 10 bytes" })`
	if got != expected {
		t.Errorf("got %q, want %q", got, expected)
	}
}
