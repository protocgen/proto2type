...
func TestTSScalarZodType(t *testing.T) {
	tests := []struct {
		...
		{protoreflect.Int64Kind, `z.union([z.string(), z.number().refine(Number.isSafeInteger, { message: "integer out of safe range" })]).pipe(z.coerce.string())`},
		{protoreflect.Uint64Kind, `z.union([z.string(), z.number().refine(Number.isSafeInteger, { message: "integer out of safe range" }).refine(n => n >= 0, { message: "must be non-negative" })]).pipe(z.coerce.string())`},
...
func TestTSWKTZodType(t *testing.T) {
	tests := []struct {
		...
		{FieldKindWrapperInt64, `z.union([z.string(), z.number().refine(Number.isSafeInteger, { message: "integer out of safe range" })]).pipe(z.coerce.string()).nullable()`},
		{FieldKindWrapperUInt32, "z.number().int().nonnegative().nullable()"},
		{FieldKindWrapperUInt64, `z.union([z.string(), z.number().refine(Number.isSafeInteger, { message: "integer out of safe range" }).refine(n => n >= 0, { message: "must be non-negative" })]).pipe(z.coerce.string()).nullable()`},
